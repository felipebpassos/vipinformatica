...
const continuePrompt =
  "Para prosseguir, precisaremos de algumas informações como nome, email e whatsapp para que um de nossos especialistas possa entrar em contato. Deseja continuar?";

export default function TicketPage() {
  const [step, setStep] = useState<
    "options" | "serviceConfirm" | "name" | "email" | "whatsapp" | "done"
  >("options");
  const [messageHistory, setMessageHistory] = useState([
    {
      content:
        "Olá! 😊 Seja muito bem-vindo(a) ao atendimento da VIP.com Informática!\n\nSe precisar de suporte técnico, orçamentos ou qualquer outra coisa, é só me avisar! Estou aqui para ajudar.",
      isUser: false,
    },
  ...
  const handleServiceSelect = (service: string) => {
    ...
    setMessageHistory((prev) => [
      ...
      {
        content:
          serviceExplanations[service as keyof typeof serviceExplanations],
        isUser: false,
      },
  ...
  const handleEmailSubmit = (email: string) => {
    ...
    setMessageHistory((prev) => [
      ...
      {
        content: "Por favor, informe abaixo seu número de whatsapp",
        isUser: false,
      },
    ]);
    setStep("whatsapp");
  ...
  const handleWhatsappSubmit = (whatsapp: string) => {
    if (!validateWhatsapp(whatsapp)) {
      setWhatsappError(
        "Por favor, insira um número de WhatsApp válido com DDD (ex: 79 9 9999-9999)."
      );
      return;
    }
    ...
    setMessageHistory((prev) => [
      ...
      {
        content:
          "Você receberá uma confirmação por e-mail e nossa equipe entrará em contato em breve para dar andamento ao seu atendimento.",
        isUser: false,
      },
    ]);
    setStep("done");
  ...
  const handleSubmitDone = () => {
    ...
    fetch("https://api.vipltda.com.br", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 pb-12 pt-40">
            ...
            return group.map((msg, msgIndex) => (
              <Message key={`${groupIndex}-${msgIndex}`} isUser={true}>
                {msg.content.split("\n\n").map((paragraph, i, paragraphs) => (
                  <p
                    key={i}
                    className={i < paragraphs.length - 1 ? "mb-2" : ""}
                  >
                    {paragraph.split("\n").map((line, j, lines) => (
                      <Fragment key={j}>
                        {line}
      ...
      {showInputOrOptions && (
        <>
          {step === "options" && (
            <Options options={services} onSelect={handleServiceSelect} />
          )}
